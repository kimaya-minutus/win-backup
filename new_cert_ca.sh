...
set -x
PaTH="/home/centos/shell_script"
echo "'$PaTH' is a path to store the generated keys and certificates"
SERVER_KEY="$PaTH/server.key"
SERVER_CSR="$PaTH/server.csr"
SERVER_CRT="$PaTH/server.crt"
EXTFILE="$PaTH/ca_variable.cnf"
OPENSSL_CMD=openssl
COMMON_NAME="$1"
...
$OPENSSL_CMD req -new -key $SERVER_KEY -out $SERVER_CSR -config $EXTFILE 2>&1 > /dev/null
if [ $? -ne 0 ] ; then
...
$OPENSSL_CMD x509 -req -days 1000 -in $SERVER_CSR -signkey $SERVER_KEY -out $SERVER_CRT 2> /dev/null
if [ $? -ne 0 ] ; then
